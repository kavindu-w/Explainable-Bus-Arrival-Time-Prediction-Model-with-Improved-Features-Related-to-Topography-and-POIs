...
run_poi = pd.read_csv('src//data//running_times_poi_final_Kandy-Digana.csv')
dwell_poi = pd.read_csv('src//data//dwell_times_poi_final_Kandy-Digana.csv')
...
def get_running_topo_features(segment):
    topo_features = ['max_elevation','avg_elevation','total_poi_count', 'average_poi_distance',
                     'segment_gradient_average', 'stop_to_stop_gradient','significant_bend_count']
    topo_feature_values = run_poi.loc[run_poi['segment']==segment ,topo_features]
    return topo_feature_values

def get_dwell_poi_features(bus_stop):
    poi_features = ['total_poi_count']
    poi_feature_values = dwell_poi.loc[dwell_poi['stop_id']==bus_stop ,poi_features]
    return poi_feature_values
...
def predict_dwell_time(bus_stop,bus):
    input_features = ['deviceid','day_of_week','time_of_day','Sunday/holiday','weekday/end','bus_stop','precip','temp',
            'dt(n-1)','dt(n-2)', 'dt(n-3)','dt(t-1)', 'dt(t-2)', 'dt(w-1)', 'dt(w-2)', 'dt(w-3)', 'rt(n-1)']
    feature_values = dwell_df.loc[(dwell_df['bus_stop']==bus_stop) & (dwell_df['deviceid']==bus),input_features]
    poi_features = get_dwell_poi_features(bus_stop)
    feature_values = pd.concat([feature_values,poi_features],axis=1)
...
def predict_running_time(bus_stop,bus):
    ...
    topo_features = get_running_topo_features(segment)
    feature_values = pd.concat([feature_values,topo_features],axis=1)
